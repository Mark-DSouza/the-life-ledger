...
- **Drift, as measured (issue #57).** The migration files were once not a faithful record of hosted: `20260805170337_eb22ffd8…` was a verbatim copy of `20260805152424_40b916f5…` plus two `GRANT`s, because Lovable's agent found `offloader_items` missing on hosted and wrote a second `CREATE TABLE` rather than reconciling. Hosted ran the later one only. That file is now reduced to its grants, which unblocked replay. Since then the two have been compared at **relation level and found to agree**: every one of the 13 tables and 109 columns the local replay produces was probed live against hosted PostgREST and exists there, hosted declares no view or enum the migrations don't create, and the local schema is identical to `src/integrations/supabase/types.ts` (which Lovable generates from the live hosted schema). The only local object absent from `types.ts` is the `touch_updated_at` trigger function, which the type generator excludes by design, not drift.
- **What that comparison cannot see.** RLS policies, indexes, constraints, defaults, triggers and grants are invisible through both PostgREST and `types.ts`. Only `bun db:diff` measures those, and it needs `bunx supabase link --project-ref yejiirdxgewpjfdintyr` first, which prompts for the hosted database password. That has still never been run.

...
The file is Google Fonts' Inter v20 partial instance, which exposes a single `wght` axis (`opsz` is pinned out), so `font-weight` drives it and there is no `font-variation-settings`. Its `cache-control` comes from a `/fonts/**` `routeRules` entry in `vite.config.ts`, because the path is fixed rather than content-hashed and Nitro's `/assets/*` rule does not reach it — so **swapping the typeface means a new filename**, not new bytes at this one.
